...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_8-4图解2个node环境下replica-shard是如何分配的" tabindex="-1"><a class="header-anchor" href="#_8-4图解2个node环境下replica-shard是如何分配的" aria-hidden="true">#</a> 8.4图解2个node环境下replica shard是如何分配的</h2><p>（1）replica shard分配：3个primary shard，3个replica shard，1 node （2）primary ---&gt; replica同步 （3）读请求：primary/replica</p><h2 id="_8-5图解横向扩容" tabindex="-1"><a class="header-anchor" href="#_8-5图解横向扩容" aria-hidden="true">#</a> 8.5图解横向扩容</h2><ul><li>分片自动负载均衡，分片向空闲机器转移。</li><li>每个节点存储更少分片，系统资源给与每个分片的资源更多，整体集群性能提高。</li><li>扩容极限：节点数大于整体分片数，则必有空闲机器。</li><li>超出扩容极限时，可以增加副本数，如设置副本数为2，总共3*3=9个分片。9台机器同时运行，存储和搜索性能更强。容错性更好。</li><li>容错性：只要一个索引的所有主分片在，集群就就可以运行。</li></ul><h2 id="_8-6-图解es容错机制-master选举-replica容错-数据恢复" tabindex="-1"><a class="header-anchor" href="#_8-6-图解es容错机制-master选举-replica容错-数据恢复" aria-hidden="true">#</a> 8.6 图解es容错机制 master选举，replica容错，数据恢复</h2><p>以3分片，2副本数，3节点为例介绍。</p><ul><li>master node宕机，自动master选举，集群为red</li><li>replica容错：新master将replica提升为primary shard，yellow</li><li>重启宕机node，master copy replica到该node，使用原有的shard并同步宕机后的修改，green</li></ul><h1 id="_9-图解文档存储机制" tabindex="-1"><a class="header-anchor" href="#_9-图解文档存储机制" aria-hidden="true">#</a> 9． 图解文档存储机制</h1><h2 id="_9-1-数据路由" tabindex="-1"><a class="header-anchor" href="#_9-1-数据路由" aria-hidden="true">#</a> 9.1． 数据路由</h2><h3 id="_9-1-1文档存储如何路由到相应分片" tabindex="-1"><a class="header-anchor" href="#_9-1-1文档存储如何路由到相应分片" aria-hidden="true">#</a> 9.1.1文档存储如何路由到相应分片</h3><p>一个文档，最终会落在主分片的一个分片上，到底应该在哪一个分片？这就是数据路由。</p><h3 id="_9-1-2路由算法" tabindex="-1"><a class="header-anchor" href="#_9-1-2路由算法" aria-hidden="true">#</a> 9.1.2路由算法</h3><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>shard = hash(routing) % number_of_primary_shards
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_11-3-type底层结构及弃用原因" tabindex="-1"><a class="header-anchor" href="#_11-3-type底层结构及弃用原因" aria-hidden="true">#</a> 11.3 type底层结构及弃用原因</h2><h3 id="_11-3-1type是什么" tabindex="-1"><a class="header-anchor" href="#_11-3-1type是什么" aria-hidden="true">#</a> 11.3.1type是什么</h3><p>type，是一个index中用来区分类似的数据的，类似的数据，但是可能有不同的fields，而且有不同的属性来控制索引建立、分词器. field的value，在底层的lucene中建立索引的时候，全部是opaque bytes类型，不区分类型的。 lucene是没有type的概念的，在document中，实际上将type作为一个document的field来存储，即_type，es通过_type来进行type的过滤和筛选。</p><h3 id="_11-3-2es中不同type存储机制" tabindex="-1"><a class="header-anchor" href="#_11-3-2es中不同type存储机制" aria-hidden="true">#</a> 11.3.2es中不同type存储机制</h3><p>一个index中的多个type，实际上是放在一起存储的，因此一个index下，不能有多个type重名，而类型或者其他设置不同的，因为那样是无法处理的</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>{
   &quot;goods&quot;: {
      &quot;mappings&quot;: {
         &quot;electronic_goods&quot;: {
            &quot;properties&quot;: {
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p><code>%{IP:clientip}</code>匹配模式将获得的结果为：clientip: 172.16.213.132 <code>%{HTTPDATE:timestamp}</code>匹配模式将获得的结果为：timestamp: 07/Feb/2018:16:24:19 +0800 而<code>%{QS:referrer}</code>匹配模式将获得的结果为：referrer: &quot;GET / HTTP/1.1&quot;</p><p>下面是一个组合匹配模式，它可以获取上面输入的所有内容：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>%{IP:clientip}\\ \\[%{HTTPDATE:timestamp}\\]\\ %{QS:referrer}\\ %{NUMBER:response}\\ %{NUMBER:bytes}
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="_20-13-2时间处理-date" tabindex="-1"><a class="header-anchor" href="#_20-13-2时间处理-date" aria-hidden="true">#</a> 20.13.2时间处理(Date)</h3><p>date插件是对于排序事件和回填旧数据尤其重要，它可以用来转换日志记录中的时间字段，变成LogStash::Timestamp对象，然后转存到@timestamp字段里，这在之前已经做过简单的介绍。 下面是date插件的一个配置示例（这里仅仅列出filter部分）：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>filter {
    grok {
